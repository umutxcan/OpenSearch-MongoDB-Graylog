...
Bu altyapı sayesinde loglar sadece okunur metinler olmaktan çıkıp filtrelenebilir, analiz edilebilir ve izlenebilir hale gelerek sağlam bir **Observability (Gözlemlenebilirlik)** temeli oluşturulmuştur.
...
* **Docker Kurulumu:**
Docker kurulumu tum sunucularda gereklidir başlanmadan once updatelerin ve docker kurulumu şarttır.
...
* **Replica Set Kontrolü:**
    Replica setini kontrol etmemizin sebebi bizim cluster yapımızı onaylamak içindir.Output olarak da 1 PRIMARY 2 SECONDARY beklenir.
...
1514/udp → syslog UDP (kullanırsan)

MongoDB URI → Graylog ayarlarını tutacağı DB
...
* **graylog UI üzrinden Kontrol:**
    ```text
    UI → Search

    Gönderilen mesaj metni aratılır

    Mesaj görünüyorsa log ingestion başarılıdır
    ```
* **Opsiyoel Pipeline ve Stream Ayarları:**
    ```text
   
   Amaç: Graylog’un merkezi işleme (processing) yeteneklerini göstermek.

    Akış:
    Pipeline → Stream → Index
    ...
    Edit connections bölümünden:

    Oluşturulan stream

    Pipeline
    arasındaki bağlantıyı kur
    ```
